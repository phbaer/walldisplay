...
Set `weather_title`, `media_title`, `buttons_title`, and `about_title` to customize titles (48 UTF-8 bytes each). The `panel_name` value is the panel's single-label hostname and is shown in the header; use 1–32 lowercase ASCII letters, numbers, or hyphens, without a leading or trailing hyphen. Existing older entries with human-readable names are migrated to the final topic component; new YAML and form values are validated strictly. The About page displays the firmware version, MQTT contract, hardware model, hostname, IPv4 address, global and link-local IPv6 addresses when available, Wi-Fi MAC, connected SSID/RSSI/channel, uptime, and reset reason. Network values refresh every five seconds and show `unavailable` until the interface has connected; credentials are never shown. The header displays the panel hostname and current page title. The clock, indicators, chips, and five footer buttons remain shared. This release provides one instance of each built-in page; it does not yet support multiple media players or multiple button pages.
...
Target hardware: ESP32-S3, 16 MB flash, octal PSRAM, 480×480 ST7701 RGB display, and GT911 touch. LVGL uses byte-swapped RGB565 in direct mode with two complete PSRAM frame buffers. The RGB panel transfers through bounded internal bounce buffers and releases a frame only after its full transfer completes, so page changes never modify the frame currently being scanned. The partition table provides read-only `appcfg`, NVS, factory and OTA slots, and SPIFFS.
