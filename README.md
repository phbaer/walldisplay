...
The public CI artifacts are signed, but they deliberately use the example configuration and therefore are **not currently ready-to-use for a new panel**: the firmware rejects the `YOUR_WIFI_SSID` placeholder and has no real broker credentials. A maintainer must publish an image built with a panel-specific `config/panel_config.yaml`, or the project must add first-boot provisioning, before an end user can flash one and configure it without a build step. Existing panels that already have a trusted signed baseline can use the application-only `.bin` through the integration's Repairs update flow. The proposed AP path is captured in the [SoftAP provisioning handover](docs/plans/softap-provisioning-handover.md).
...
The firmware, blueprint, custom integration, and Python project release is `1.0.0`; MQTT contract `7`. Both synchronization paths publish retained compatibility metadata to `<base>/set/blueprint_info`:

```json
{"version":"1.0.0","contract":"7"}
```
...
| Commands | `<base>/cmd/...` | Non-retained buttons, media, sync, configuration, OTA, and screenshots. |
...
Target hardware: ESP32-S3, 16 MB flash, octal PSRAM, 480×480 ST7701 RGB display, and GT911 touch. LVGL uses byte-swapped RGB565 with synchronized direct-mode double buffering and VSYNC frame-boundary swaps, so screen transitions redraw only invalidated regions while keeping both buffers coherent. The partition table provides read-only `appcfg`, NVS, factory and OTA slots, and SPIFFS.

Use the discovered **Panel Update Manifest URL** entity or publish a non-retained HTTPS manifest URL to `<base>/cmd/update`. OTA is available only in builds with ESP-IDF app-signature verification enabled. The panel checks the HTTPS manifest SHA-256 and size and lets ESP-IDF verify the signed image before selecting its boot partition. A pending image has 120 seconds to connect to MQTT; otherwise it explicitly reboots for rollback. Large OTA buffers are worker-owned static storage instead of task-stack allocations; logs report stack low-water headroom after each download.
...
The repository's Python tools and tests use the standard library plus the dependencies supplied by the Home Assistant test environment; `uv` is optional and is not required to build firmware or run the documented commands. `pyproject.toml` is the canonical project-version source. After changing its `MAJOR.MINOR.PATCH` value, run `python tools/sync_project_version.py` and commit the generated updates to the firmware header, both integration manifests and compatibility payloads, and the blueprint metadata. Forgejo and GitHub workflows verify these checked-in references before building or packaging and fail if they drift. The current aligned project version is `1.0.0`; ESP-IDF and its Python environment remain separate prerequisites for firmware builds.
