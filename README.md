...
The Forgejo release workflow builds every push. A `v*` tag also creates a release containing the application image, an OTA manifest, and a factory archive. Re-running a tagged release (including manual dispatch for that tag) replaces the existing same-tag release with newly built assets. Tagged release jobs install Node.js, `jq`, and `curl` in the build container for the Forgejo release action, so the runner needs package-repository access. Use an HTTPS manifest URL with `<base>/cmd/update` or the **Panel Update Manifest URL** entity. Do not retain the command.
