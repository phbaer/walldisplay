...
Media commands are `cmd/media/previous`, `play_pause`, `next`, `volume_down`, `volume_up`, `volume`, and `favoriteN`. The media page uses a narrow blue vertical volume rocker pinned to the right edge: its transparent `+` and `−` ends support a tap for a small adjustment or press-and-hold repeat, while its central slider spans the full media-content height down to the favourites row and publishes `cmd/media/volume` with an integer percentage from 0 to 100. It follows the `volume_level` media-state field. The transport row is left aligned to keep clear of the rocker, and the favourites row reserves its right edge. The media page uses a 136 px cover-art area and a separate metadata column; favourites publish retained labels and icons, and their play-media JSON targets the selected player. `walldisplay_sync` fetches the selected player's artwork, converts it to a bounded JPEG, and serves it from a random-token panel URL; configure an internal or external Home Assistant URL reachable from the panel. MQTT Sync remains a no-custom-code alternative and forwards its media URL directly, so its source must already be a reachable baseline JPEG.
