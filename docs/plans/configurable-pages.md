...
Selected MQTT contract: **6** (new layout and grid topics and page semantics).
Keep these values for every refinement on this branch.

## Scope and implementation order

1. Introduce a bounded layout model: one weather, media, and buttons page; ordered enabled slots, configurable titles, and startup page. Validate before saving, preserve the last valid configuration, migrate the previous startup preference, and reuse a saved layout on reboot.
...
- One instance of each built-in page. Multiple rooms/media sources and dynamically allocated page instances are future work.
