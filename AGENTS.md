...
4. For a firmware or Home Assistant blueprint release, keep `APP_FW_VERSION` and the Home Assistant blueprint version aligned. Bump the MQTT contract version in both places whenever MQTT topics, payloads, discovery entities, or their semantics change.
   - At the start of a feature branch, inspect its base revision and choose one release version and, if required, one MQTT contract version for the whole feature. Record and use those exact values consistently in every commit on that branch.
   - For one cohesive feature branch, change the release and MQTT contract versions at most once from the base revision. Never increment them again for follow-up commits, fixes, refinements, or documentation updates. If a later change needs a different semantic-version classification or MQTT contract, split it into a new feature branch rather than revising the current feature's selected versions.
...
7. Commit each closed implementation step when it is appropriate to do so. Use a short, descriptive commit message; keep unrelated work in separate commits and do not commit incomplete or unverified changes.
