...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    async def publish_weather(_: Event | None = None) -> None:
        ...
        try:
            ...
            forecast = response.get(weather_entity, {}).get("forecast", [])[:3]
        ...
        payload = {"condition": state.state, "temperature": attrs.get("temperature"), "forecast": [
            {"day": str(item.get("datetime", ""))[0:3], "condition": item.get("condition", "unknown"), "high": item.get("temperature"), "low": item.get("templow", item.get("temperature"))} for item in forecast]}
